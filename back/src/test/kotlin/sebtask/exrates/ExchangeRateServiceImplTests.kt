...
class ExchangeRateServiceImplTests {
    ...
    fun `should correctly parse exchange rate history XML`() {
        ...
        val result: List<ExchangeDay> = service.fetchRatesHistory()

        assertEquals(2, result.size)

        assertEquals("2024-02-26", result[0].date)
        assertEquals(2, result[0].rates.size)
        assertEquals(ExchangeRate("USD", 1.085), result[0].rates[0])
        assertEquals(ExchangeRate("GBP", 0.862), result[0].rates[1])

        assertEquals("2024-02-25", result[1].date)
        assertEquals(2, result[1].rates.size)
        assertEquals(ExchangeRate("USD", 1.083), result[1].rates[0])
        assertEquals(ExchangeRate("GBP", 0.860), result[1].rates[1])
    }
}
